<template>
  <div>
    <h2 class="text-center">Información de la Mujer Embarazada: {{ formData.nombres.toLocaleUpperCase() }} {{ formData.apellidos.toLocaleUpperCase() }} </h2>
...
export default {
  ...
  mounted() {
    // Cargar la información de la mujer al montar el componente
    this.loadMujerData();
  },
  methods: {
    loadMujerData() {
      axios.get(`${BASE_URL}/api/mujer/${this.mujerId}`, {
        headers: {
      ...
      .catch((error) => {
        console.error('Error al cargar la información de la mujer:', error);
      });
    },
    submitForm() {
    ...
    if (this.formData.discapacidad === 'No') {
      ...
      axios.put(`${BASE_URL}/api/mujer/${this.mujerId}`, this.formData, {
        headers: {
          'x-access-token': this.$store.state.auth.user.accessToken,
        },
      })
      .then(() => {
        swal({
          ...
          text: 'La información de la madre ha sido actualizada correctamente.',
          icon: 'success',
          button: 'OK',
        });
      })
